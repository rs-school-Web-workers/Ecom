...
import { destroyClient, getCart, isLogged } from '../../utils/api/Client';
...
const BASKETSVG = `<svg width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default class Header {
  ...
  createNavigateContainer() {
    ...
    const basketLink: HTMLAnchorElement = this.createNavigateLink(headerLinkNames.BASKET, PagePath.BASKET);
    ...
    basketLink.innerHTML = `Basket${BASKETSVG}`;
    window.addEventListener('cart-update', async () => {
      basketLink.innerHTML = `Basket(${(await getCart())?.body.totalLineItemQuantity ?? 0})${BASKETSVG}`;
    });
    window.dispatchEvent(new Event('cart-update'));
    basketLink.classList.add(headerClassNames.BASKET_LINK);
