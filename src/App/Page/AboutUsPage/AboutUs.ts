...
export class AboutUsPage extends Page {
  ...
  init() {
    ...
    this.createAbout();
    data.forEach(({ img, shortBio, name, role, ghLink }) => {
      this.createCard(img, name, role, shortBio, ghLink);
    });
  }
  ...
  createCard(img: string, name: string, role: string, shortBio: string, ghLink: string) {
    const card = new Component('div', [aboutUsTeamCard]);
    const cardImg = new Component('img', [aboutUsTeamCardImg]).getElement<HTMLImageElement>();
    cardImg.src = img;
    cardImg.alt = `${name} photo`;
    const cardName = new Component('h2', [aboutUsTeamCardName]);
    ...
    const cardDescriptionRole = new Component('h3', []);
    cardDescriptionRole.setTextContent(role);
    const cardDescriptionLinkGithub = new Component('a', []).getElement<HTMLAnchorElement>();
    cardDescriptionLinkGithub.innerHTML = GITHUBSVG;
    cardDescriptionLinkGithub.href = ghLink;
    cardDescriptionLinkGithub.target = '_blank';
    const cardDescriptionShortBio = new Component('p', []);
    cardDescriptionShortBio.setTextContent(shortBio);
    cardDescriptionContainer.setChildren(
      cardDescriptionName.getElement(),
      cardDescriptionRole.getElement(),
      cardDescriptionLinkGithub,
      cardDescriptionShortBio.getElement()
    );
    card.setChildren(cardImg, cardName.getElement(), cardRole.getElement(), cardDescriptionContainer.getElement());
    this.wrapperTeam.setChildren(card.getElement());
  }
}
